...
test('a full tank represents the configured 22,500-world-unit range', () => {
  assert.ok(FULL_TANK_RANGE_WORLD_UNITS >= 20_000 && FULL_TANK_RANGE_WORLD_UNITS <= 25_000);
  let fuel = 100;
  for (let distance = 0; distance < FULL_TANK_RANGE_WORLD_UNITS; distance += 100) {
    fuel = consumeFuelForDistance(fuel, Math.min(100, FULL_TANK_RANGE_WORLD_UNITS - distance));
  }
  assert.ok(fuel < 1e-9);
});
